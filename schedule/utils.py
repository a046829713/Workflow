...
def convert_str_datetime(ddatetime:str):
    """
...
def convert_to_minutes(delta):
    # 將年、月、日和毫秒轉換為分鐘
    minutes = delta['years'] * 365 * 24 * 60  # 假設一年有365天
    minutes += delta['months'] * 30 * 24 * 60  # 假設一個月有30天
    minutes += delta['days'] * 24 * 60
    minutes += delta['milliseconds'] / (1000 * 60)  # 將毫秒轉換為分鐘
    return minutes